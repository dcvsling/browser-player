...
export class AuthClient {
  ...
  RequestAccessToken(res: GetCodeWithPKCEResponse): Promise<string> {
    const params = [
      ...
      ["code", res.code],
      ["state", this.sessionStorage?.getItem(STATE)!],
  ...
  async RequestCodeWithPKCE() {
    const expire = this.sessionStorage?.getItem(CODE_WITH_PKCE_EXPIRE);
    const options = this.options;
    if(!expire || parseFloat(expire) < Date.now()) {
      ...
      this.sessionStorage?.setItem(REDIRECT_TO_RESTORE_STATE, this.route.snapshot.url.reduce((ctx, x) => ctx + '/' +  x.path, ''));
      if(this.location)
        this.location.href = ApiRequest.Authorize.endpoint + '?' + params;
      return of();
    } else {
      return await this.RequestAccessToken(JSON.parse(this.sessionStorage?.getItem(CODE_WITH_PKCE)!));
    }
  }
